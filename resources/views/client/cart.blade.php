                            ...
                            <tbody>
                                @foreach ($cart as $key => $item)
                                    <tr class="prod-{{ $item->pivot->id }}">
                                        ...
                                        <td>
                                            <div class="prod-qty"><input type="number" class="quantity" data-id="{{ $item->id }}" data-key="{{ $key }}" data-price="{{ $item->selling_price }}" data-quantity="{{ $item->quantity }}" value="{{ $item->pivot->quantity > $item->quantity ? $item->quantity :  $item->pivot->quantity }}" max="{{ $item->quantity ?? null}}">
                                                pcs
                                                @if ($item->quantity)
                                                    <p>(< {{ $item->quantity }})</p>
                                                @endif
                                        ...
                                        <td><div class="prod-price" style="width: 100px;"><strong class="totalPrice mark-price">{{ $item->selling_price * $item->pivot->quantity }}</strong></div></td>
    ...
    <script>
        ...
        $('.get-promotion').on('click', function() {
            $.ajax({
                ...
                if(!arrPromotion.includes($('#voucher').val())) {
                    arrPromotion.push($('#voucher').val())
                }
                arrPromotion.forEach(element => {
                    $('.get-promotion').after(' <span class="label label-default promo-text">'+element+'</span>')
                });
                $('#voucher').val('')

                var href = "{{ route('client.shippingAddress', ['voucher' => '__VOUCHER__' ]) }}"
                href = href.replace(/__VOUCHER__/g, arrPromotion);
        ...
        $('.quantity').on('change', function(){
            ...
            $('.totalPrice:eq('+$(this).data("key")+')').text($(this).val() * $(this).data('price')).unmask().mask('#.##0', { reverse: true }).append(' đ')
            $(".totalPrice").unmask();
            var sumPrice = 0;
            $(".totalPrice").each(function() {
                sumPrice += parseInt($(this).text());
            });
            $(".totalPrice").mask('#.##0', { reverse: true }).append(' đ');
            $('#total-price').text(sumPrice).unmask().mask('#.##0', { reverse: true }).append(' đ');

            $.ajax({
                url : "{{ route('client.ajaxUpdateQuantityCart') }}",
                method: 'GET',
                data: {
                    id: $(this).data('id'),
                    quantity: $(this).val(),
