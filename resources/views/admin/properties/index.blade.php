...
@section('content')
    ...
    <div class="row justify-content-between mb-4">
        <div class="col-4">
            <h1 class="h3 text-gray-800">Properties</h1>
        </div>
        <div class="col-6 text-right">
            <button type="button" class="btn btn-success" data-toggle="modal" data-target="#addPropertyModal"><i class="fa fa-plus"></i> Add</button>
            <button type="button" id="btn-remove-all" data-url="{{url(route('admin.properties.deleteMultiple'))}}" class="btn btn-danger"><i class="fa fa-trash" ></i> {{trans('product.delete_selected_item')}}</button>
        </div>
    </div>
        ...
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('admin.properties.store') }}" method="POST" id="add-property">
                    @csrf
            ...
            <div class="modal-footer">
              ...
              <button type="submit" class="btn btn-primary" form="add-property">Submit</button>
            </div>
          </div>
        </div>
    </div>
    ...
    {{-- Modal Update --}}
    <div class="modal fade" id="updatePropertyModal" tabindex="-2" role="dialog" aria-labelledby="updatePropertyModalLabel">
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            <div class="modal-header">
            ...
            <div class="modal-body">
                <form action="{{ route('admin.properties.update') }}" method="POST" id="update-property">
                    {{ method_field('PUT') }}
                    @csrf
                    <input type="hidden" id="idProperty" name="id" value="">
                    <div class="form-group">
                        <label for="name" class="control-label">Name:</label>
                        <input type="text" class="form-control" id="nameProperty" name="name">
                    </div>
                    <div class="form-group">
                        <label for="noteProperty" class="control-label">Note:</label>
                        <textarea class="form-control" id="noteProperty" name="note"></textarea>
                    </div>
                </form>
            </div>
            <div class="modal-footer">
              <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
              <button type="submit" class="btn btn-primary" form="update-property">Submit</button>
            </div>
          </div>
        </div>
    </div>
    ...
    var table = $('#datatable').DataTable({
            ...
            columns: [
                ...
                { data: 'created_at', name: 'created_at'},
                {
                    ...
                    "className": "text-center",
                    "render": function(data, type, row, meta){
                        return  '<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#updatePropertyModal" data-name="'+ row.name +'" data-note="'+ row.note +'" data-id="'+ row.id +'" ><i class="fas fa-edit"></i> Edit</button>';
                    }
                },
            ],
        });
        ...
        $('#updatePropertyModal').on('show.bs.modal', function (event) {
            ...
            var modal = $(this)
            modal.find('#idProperty').val(id)
            modal.find('#nameProperty').val(name)
